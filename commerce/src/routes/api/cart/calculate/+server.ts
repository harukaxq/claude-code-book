...
import { z } from 'zod';
import { calculateCart, CheckoutError } from '$lib/server/services/checkout';
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		return json(await calculateCart(parsed.data.items, parsed.data.couponCode ?? null));
	} catch (error) {
		...
		console.error(error);
		return json({ message: '金額計算中にエラーが発生しました。' }, { status: 500 });
	}
};
