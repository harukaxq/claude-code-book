...
購入者は商品をカートへ追加し、固定額またはパーセントクーポンを適用して注文できる。運営者は商品、クーポン、注文を管理できる。
...
商品名または商品画像から商品詳細画面へ遷移できる。

### 3.2 商品詳細 `/products/:productId`

公開中の商品情報、平均評価、レビュー件数、レビュー一覧を表示する。レビューは新しいものから表示する。

固定デモユーザーが対象商品を購入済みで、まだレビューを投稿していない場合は、評価とコメントの投稿フォームを表示する。レビューがない場合は、その旨を表示する。非公開商品は表示しない。
...
- `code`は一意とする
- 固定額クーポンの`discountValue`は円単位の正の整数とする
- パーセントクーポンの`discountValue`は1以上100以下の整数とする
...
- `rating`は1以上5以下の整数とする
- `comment`は前後の空白を除いて1文字以上200文字以下とする
- 同じ購入者は1商品につき1件だけレビューできる
- 平均評価とレビュー件数は保存せず、レビューから計算する
...
パーセントクーポンは商品小計全体へ適用し、1円未満を切り捨てる。

```text
クーポン割引額 = floor（商品小計 × 割引率 ÷ 100）
```
...
- 固定デモユーザー`demo-customer`として投稿する
- 注文履歴に対象商品が含まれる場合だけ投稿できる
- 同じ購入者が同じ商品へ2件目を投稿することはできない
- レビューの編集、削除、管理者による承認は扱わない
- 評価範囲、コメント長、購入履歴、重複投稿はサーバー側で検証する
