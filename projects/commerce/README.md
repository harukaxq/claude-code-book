...
TinyCommerceは、SvelteKitで作った小さなECアプリです。商品をカートへ入れ、クーポンを使って注文できます。購入済みの商品にはレビューを投稿できます。商品・クーポン・注文を確認する管理画面も含みます。

SvelteKitの画面からサービス、Drizzle ORM、SQLiteへ処理が流れる様子を追いやすいように、意図的に小さな構成にしています。
...
`bun run dev`はSeaweedFSの起動、SQLiteのマイグレーション、初期データと画像の投入、SvelteKitの起動を順番に行います。`.env`の作成は不要です。
...
docs/spec.md                  アプリの仕様書
```

## 開発用コマンド

すべてこのディレクトリ直下から実行します。
...
詳しい要件は[`docs/spec.md`](docs/spec.md)にあります。
